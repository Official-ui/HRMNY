...
function screenLaunch() {
  return scr('launch', 'bare',
    ...
    `<div style="margin-top:22px">` +
      roleCard('discover', ART.violet, "I'm a brand / creator", 'Discover & license tracks') +
      roleCard('studio',   ART.green,  "I'm an artist", 'Upload, prove & get paid') +
    `</div>` +
    `<p class="note-line" style="text-align:center;margin-top:24px">Prototype · sample data · no real payments</p>`);
}
const roleCard = (to, art, t, sub) =>
  `<label class="acard rolecard" for="scr-${to}"><span class="tart" style="width:48px;height:48px;background:${art}"></span>` +
...
function screenUp1() {
  return scr('up1', '',
    ...
    `<label class="dropzone" for="scr-up2" style="margin-top:16px"><div class="dz-ic">⤓</div>` +
...
function screenVerify() {
  return scr('verify', '',
    ...
    `<div class="scanbox" style="margin-top:18px"><div class="scan-target" style="background:${ART.violet}"></div>` +
...
function screenReport() {
  ...
  return scr('report', '',
    ...
    `<div class="acard" style="margin:14px 0 0;display:flex;gap:12px;align-items:center">` +
      `<span class="tart" style="width:46px;height:46px;background:${ART.violet}"></span>` +
...
function screenWallet() {
  ...
  return scr('wallet', '',
    ...
    `<div class="acard" style="margin-top:10px;padding:6px 16px">` +
      txn('↓', '“Midnight Drive” · Paid Social', 'Licensed by Lumen Studios', '+$343', 'in') +
      txn('↓', '“Paper Cranes” · Organic', 'Licensed by @veracreates', '+$120', 'in') +
      txn('↑', 'Payout to •••• 4471', 'May 30', '−$1,540', 'out') +
      txn('↓', '“Midnight Drive” · Broad + Exclusive', 'Licensed by Atlas Athletic', '+$2,592', 'in') +
    `</div>` +
